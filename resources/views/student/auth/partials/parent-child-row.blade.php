...
<div class="child-row p-4 bg-gray-50 rounded-xl space-y-3 {{ $hidden ? 'hidden' : '' }}" data-index="{{ $index }}">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
            ...
            <input type="number" name="students[{{ $index }}][birth_year]" value="{{ $s['birth_year'] ?? '' }}" min="1990" max="{{ date('Y') }}" required dir="ltr" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
        </div>
        <div>
            ...
            <select name="students[{{ $index }}][grade]" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                ...
                @for($g = 1; $g <= 12; $g++)
                    <option value="{{ $g }}" {{ ($s['grade'] ?? '') == $g ? 'selected' : '' }}>{{ __('parent-signup.child.grade_option', ['grade' => $g]) }}</option>
                @endfor
            </select>
        </div>
    </div>
    ...
    <div class="student-separate-fields space-y-3 pt-2 border-t border-gray-200" style="{{ ($s['login_type'] ?? '') === 'separate' ? '' : 'display:none' }}">
        ...
        <div class="student-contact-phone flex gap-2" style="{{ ($s['contact_type'] ?? '') === 'phone' ? '' : 'display:none' }}">
            <select name="students[{{ $index }}][phone_prefix]" dir="ltr" class="w-28 px-3 py-2 border border-gray-300 rounded-lg student-phone-prefix">
